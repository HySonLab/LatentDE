...
class ESM2_Landscape(LightningModule):
    def __init__(self,
                 net: torch.nn.Module,
    ...
    def infer_fitness(self, sequence: List[str], device: torch.device):
        with torch.inference_mode():
            tokenized_seq = self.net.tokenizer(sequence,
                                               add_special_tokens=True,
                                               return_tensors="pt",
                                               padding=True).to(device)
            outputs = self.net.esm(**tokenized_seq).last_hidden_state
            fitness = self.net.decoder(outputs)
            return fitness.detach().cpu().squeeze().tolist()

    def infer_fitness_glob(self, sequence: List[str], device: torch.device, batch: int):
        seqs = [sequence[i:i + batch] for i in range(0, len(sequence), batch)]
        scores = []
        for seq in seqs:
            fitness = self.infer_fitness(seq, device)
